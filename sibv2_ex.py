...
def sibv_mix(vec: np.ndarray, better_vec: np.ndarray) -> np.ndarray:
    ''' Returns an array (vec) that contains a percentage of values 
        ...
        ind: the index values to exchange in random order    
        r1: a random value between min_val & max_val
        r2: a random value that meets constraints '''
    
    new_pos = vec.copy()
    percent_to_replace = 0.30
    qb = int(np.ceil(vec.size * percent_to_replace)) 
    inds = np.random.choice(np.arange(new_pos.size), qb, replace=False) 
    for ind in inds:
    ...
    assert feasible_vec(new_pos), 'sibv_mix() returned an unfeasible array'
    return new_pos
...
if __name__ == '__main__':
    ...
    experiment(optimise_func=optimize, split_particles_list=split_particles_list, experiment_name='sibv2_ex_30')
