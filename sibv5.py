...
import time
from pso_ex import PSO, feasible_vec, calculate_profit, experiment, split_particles_list, poss_val, random_val
...
def sibv_mix2(vec: np.ndarray, better_vec: np.ndarray, percent_to_replace:int) -> np.ndarray:
    ''' Returns an array (vec) that contains a percentage of values 
        ...
        diff_indicies: the index positions where vec and better_vec are different 
        indices: qb number of values from diff_indices randomly sampled '''
    ...
    diff_indicies = np.where(vec != better_vec)[0]
    if qb < diff_indicies.size:
        indices = np.random.choice(diff_indicies, size=qb, replace=False)
    else:
        # qb > diff_indicies.size   
        indices = diff_indicies
    
    for ind in indices:
        min_val = np.minimum(better_vec[ind], vec[ind])
        max_val = np.maximum(better_vec[ind], vec[ind])
...
class SIBV5(PSO):
    def mix(self):
        for particle in self.particles:
            particle['mixwLB_pos'] = sibv_mix2(vec=particle['position'], better_vec=particle['lbest_pos'], percent_to_replace=0.30)
            particle['mixwLB_val'] = calculate_profit(particle['mixwLB_pos'])
            particle['mixwGB_pos'] = sibv_mix2(vec=particle['position'], better_vec=self.gbest_pos, percent_to_replace=0.20)
...
if __name__ == '__main__':
    ...
    optimize.counter = 0
    experiment(optimise_func=optimize, split_particles_list=split_particles_list, experiment_name='sibv5_30_20_rand')